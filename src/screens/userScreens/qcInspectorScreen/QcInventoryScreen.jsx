...
const QcInventoryScreen = () => {
    ...
    const loadData = useCallback(async () => {
        ...
        logScreenApi("QcInventoryScreen", "products", res, buildUrl("products", "limit=100"));
        if (isApiSuccess(res)) {
    ...
    return (
        <View style={styles.root}>
            <MyHeader title="Inventory QC" onBackPress={navigation.goBack} />
            <SafeAreaView style={styles.safeArea} edges={[]}>
                <FlatList
                    ...
                    ListHeaderComponent={<Text style={styles.pageTitle}>Inventory QC</Text>}
                    renderItem={({ item }) => (
                        <View style={styles.card}>
                            <Text style={styles.title}>{item.product} — {item.size}</Text>
                            <Text style={styles.meta}>SKU: {item.sku || "—"} • Rack: {item.rack}</Text>
                            <Text style={[styles.stock, item.low && styles.stockLow]}>Stock: {item.stock}</Text>
                        </View>
                    )}
                    ListEmptyComponent={<Text style={styles.empty}>No inventory data</Text>}
                />
            </SafeAreaView>
        </View>
    );
...
export default QcInventoryScreen;
