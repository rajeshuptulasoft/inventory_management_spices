...
const QcProfileScreen = () => {
    ...
    const loadProfile = useCallback(async () => {
        ...
        logScreenApi("QcProfileScreen", "auth/profile", res, buildUrl("auth/profile"));
        const data = isApiSuccess(res) ? extractApiData(res) : stored?.user || stored;
        const name = data?.name || data?.full_name || stored?.name || "";
        const initials = name ? name.charAt(0).toUpperCase() : "Q";
        setProfile({
            name,
            ...
            department: data?.department || "Quality Control",
    ...
    return (
        <View style={styles.root}>
            <FinanceHeader
                title="Profile"
                profileInitial={profile.initials || "Q"}
...
export default QcProfileScreen;
