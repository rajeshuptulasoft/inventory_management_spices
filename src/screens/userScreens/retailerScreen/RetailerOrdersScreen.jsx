...
const RetailerOrdersScreen = () => {
    ...
    const loadData = useCallback(async () => {
        ...
        logScreenApi("RetailerOrdersScreen", "orders", res, buildUrl("orders", "limit=100"));
        if (isApiSuccess(res)) {
            setOrders(
    ...
    const filtered = useMemo(() => {
        ...
        return orders.filter(
            (o) => o.orderId.toLowerCase().includes(q) || o.status.toLowerCase().includes(q)
        );
    }, [orders, search]);

    return (
        <View style={styles.root}>
            <FinanceHeader title="Orders" profileInitial="R" onProfilePress={navigation.openDrawer} />
