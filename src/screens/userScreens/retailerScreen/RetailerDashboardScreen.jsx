...
const buildSummary = (orders = [], collections = []) => {
    ...
    return [
        ...
        },
        {
            id: "2",
            label: "COLLECTIONS",
            value: fmtInr(totalCollection),
            footer: `${collections.length} records`,
            icon: "💵",
            iconBg: "#DCFCE7",
        },
        {
...
const RetailerDashboardScreen = () => {
    ...
    const fetchDashboard = useCallback(async () => {
        const [ordersRes, collectionsRes] = await Promise.all([
            ...
            GETNETWORK(buildUrl("collections", "limit=100"), true),
        ]);
        logScreenApi("RetailerDashboardScreen", "orders", ordersRes, buildUrl("orders", "limit=100"));
        logScreenApi("RetailerDashboardScreen", "collections", collectionsRes, buildUrl("collections", "limit=100"));

        const orders = isApiSuccess(ordersRes) ? extractApiList(ordersRes).map(mapSalesOrderRow) : [];
        const collections = isApiSuccess(collectionsRes) ? extractApiList(collectionsRes) : [];
        setSummary(buildSummary(orders, collections));
        setRefreshing(false);
    }, []);
    ...
    return (
        <View style={styles.root}>
            ...
            <SafeAreaView style={styles.safeArea} edges={[]}>
                ...
                >
                    <Text style={styles.title}>Retailer Overview</Text>
                    <Text style={styles.subtitle}>Orders, collections & schemes</Text>
