...
const RetailerSchemesScreen = () => {
    ...
    const loadData = useCallback(async () => {
        const res = await GETNETWORK(buildUrl("schemes", "limit=100"), true);
        logScreenApi("RetailerSchemesScreen", "schemes", res, buildUrl("schemes", "limit=100"));
        if (isApiSuccess(res)) {
            setRows(
                extractApiList(res).map((row, i) => ({
                    id: String(row.id ?? i),
                    name: row.name || row.scheme_name || "Scheme",
                    type: row.scheme_type || row.type || "—",
    ...
    return (
        <View style={styles.root}>
            <FinanceHeader title="Schemes" profileInitial="R" onProfilePress={navigation.openDrawer} />
            <SafeAreaView style={styles.safeArea} edges={[]}>
                <FlatList
                    ...
                    ListHeaderComponent={
                        <View>
                            <Text style={styles.pageTitle}>Available Schemes</Text>
                            <TextInput
                                style={styles.search}
                    ...
                    ListEmptyComponent={<Text style={styles.empty}>No schemes found</Text>}
                />
            </SafeAreaView>
        </View>
    );
...
export default RetailerSchemesScreen;
