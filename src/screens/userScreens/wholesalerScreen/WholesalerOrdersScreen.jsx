...
const WholesalerOrdersScreen = () => {
    ...
    const loadData = useCallback(async () => {
        ...
        logScreenApi("WholesalerOrdersScreen", "orders", res, buildUrl("orders", "limit=100"));
        if (isApiSuccess(res)) {
            setOrders(
                extractApiList(res).map((row) => {
                    ...
                    return {
                        ...
                        customer: row.customer_name || row.party_name || base.distributor,
                        amount: base.amount,
                        status: base.status,
                        date: base.date,
                        items: base.items,
    ...
    return (
        <View style={styles.root}>
            <FinanceHeader title="Orders" profileInitial="W" onProfilePress={navigation.openDrawer} />
            <SafeAreaView style={styles.safeArea} edges={[]}>
                <FlatList
                    ...
                    ListHeaderComponent={
                        <View>
                            <Text style={styles.pageTitle}>Wholesale Orders</Text>
                            <TextInput
                                style={styles.search}
