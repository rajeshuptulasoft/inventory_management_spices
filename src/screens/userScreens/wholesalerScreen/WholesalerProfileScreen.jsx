...
const WholesalerProfileScreen = () => {
    ...
    const loadProfile = useCallback(async () => {
        ...
        logScreenApi("WholesalerProfileScreen", "auth/profile", res, buildUrl("auth/profile"));
        const data = isApiSuccess(res) ? extractApiData(res) : stored?.user || stored;
        const name = data?.name || data?.full_name || stored?.name || "";
        setProfile({
            name,
            role: data?.role?.role_name || stored?.apiRole || stored?.role || "Wholesaler",
            ...
            initials: name ? name.charAt(0).toUpperCase() : "W",
        });
        setLoading(false);
    }, []);
...
export default WholesalerProfileScreen;
