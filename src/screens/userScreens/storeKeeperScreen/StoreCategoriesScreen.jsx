...
const MachineCategoriesScreen = () => {
    ...
    const fetchCategories = useCallback(async (isRefresh = false) => {
        ...
        try {
            ...
            logScreenApi("MachineCategoriesScreen", "categories", res, buildUrl("categories"));
            if (!isApiSuccess(res)) {
    ...
    const handleSaveCategory = async (form) => {
        ...
        if (form.id) {
            ...
            logScreenApi("MachineCategoriesScreen", "categories/update", res, buildUrl(`categories/${form.id}`));
        } else {
            res = await POSTNETWORK(buildUrl("categories"), payload, true);
            logScreenApi("MachineCategoriesScreen", "categories/create", res, buildUrl("categories"));
        }
        if (!isApiSuccess(res)) {
        ...
        Alert.alert("Delete Category", `Delete "${item.name}"?`, [
            { text: "Cancel", style: "cancel" },
            {
                ...
                onPress: async () => {
                    ...
                    logScreenApi("MachineCategoriesScreen", "categories/${item.id}", res, buildUrl(`categories/${item.id}`));
                    if (!isApiSuccess(res)) {
