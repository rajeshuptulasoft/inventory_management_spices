...
const DistributorCollectionsScreen = () => {
    ...
    const loadData = useCallback(async () => {
        const res = await GETNETWORK(buildUrl("collections"), true);
        logScreenApi("DistributorCollectionsScreen", "collections", res, buildUrl("collections"));
        if (isApiSuccess(res)) {
            setRows(
                extractApiList(res).map((row, i) => ({
                    id: String(row.id ?? i),
                    party: row.party?.name || row.party_name || row.distributor?.name || row.distributor_name || "—",
    ...
    const filtered = useMemo(() => {
        ...
        if (!q) return rows;
        return rows.filter((r) => r.party.toLowerCase().includes(q) || r.status.toLowerCase().includes(q));
    }, [rows, search]);

    return (
        <View style={styles.root}>
            <FinanceHeader title="Collections" profileInitial="D" onProfilePress={navigation.openDrawer} />
                ...
                <FlatList
                    ...
                    ListHeaderComponent={
                        <View>
                            ...
                            <TextInput
                                style={styles.search}
                                placeholder="Search party, status..."
...
export default DistributorCollectionsScreen;
