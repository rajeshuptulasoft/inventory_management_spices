...
const mapParty = (row) => ({
    id: String(row.id),
    name: row.name || row.party_name || "",
    code: row.code || row.party_code || `P-${row.id}`,
    city: row.city || row.address?.city || "—",
    status: (row.status || "active").toString(),
});

const DistributorPartiesScreen = () => {
    ...
    const loadData = useCallback(async () => {
        ...
        logScreenApi("DistributorPartiesScreen", "parties", res, buildUrl("parties", "limit=200"));
        if (isApiSuccess(res)) setRows(extractApiList(res).map(mapParty));
        setRefreshing(false);
    }, []);
    ...
    return (
        <View style={styles.root}>
            <FinanceHeader title="Parties" profileInitial="D" onProfilePress={navigation.openDrawer} />
...
export default DistributorPartiesScreen;
