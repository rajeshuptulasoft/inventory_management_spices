...
const AddSuperStockistModal = ({ visible, onClose, onSave }) => {
                ...
                >
                    <View style={styles.modalCard}>
                        <View style={styles.modalHeader}>
                            <Text style={styles.modalTitle}>Add Super</Text>
                            <TouchableOpacity
                                onPress={handleClose}
...
const StockistCard = ({ item, onView, onEdit }) => (
...
const SuperStockListScreen = () => {
    const navigation = useFinanceNavigation();
    const [stockists, setStockists] = useState(INITIAL_STOCKISTS);
    ...
    const handleSaveStockist = (form) => {
        const newStockist = {
            ...
            gstin: form.gstin,
            pan: form.pan,
            credit: formatCurrency(form.creditLimit),
            outstanding: "₹0",
            status: form.status,
        };
    ...
    const handleView = (item) => {
        Alert.alert("View Super Stockist", `${item.name} (${item.code})`);
    };

    const handleEdit = (item) => {
        Alert.alert("Edit Super Stockist", `Edit ${item.name}`);
    };
        ...
        <View style={styles.root}>
            ...
            <SafeAreaView style={styles.safeArea} edges={[]}>
                <FlatList
                    ...
                    renderItem={({ item }) => (
                        <StockistCard item={item} onView={handleView} onEdit={handleEdit} />
                    )}
...
export default SuperStockListScreen;
