...
const AsmProfileScreen = () => {
    ...
    const loadProfile = useCallback(async () => {
        ...
        logScreenApi("AsmProfileScreen", "auth/profile", res, buildUrl("auth/profile"));
        const data = isApiSuccess(res) ? extractApiData(res) : stored?.user || stored;
        const name = data?.name || data?.full_name || stored?.name || "";
        setProfile({
            name,
            role: data?.role?.role_name || stored?.apiRole || stored?.role || "Area Sales Manager",
            userId: String(data?.id ?? stored?.userId ?? ""),
            email: data?.email || stored?.email || "",
            phone: data?.phone || data?.mobile || "—",
            department: data?.department || "Field Sales",
            joinedOn: data?.created_at?.slice?.(0, 10) || "—",
            initials: name ? name.charAt(0).toUpperCase() : "A",
        });
        setLoading(false);
    }, []);
...
export default AsmProfileScreen;
