...
const AsmVisitsScreen = () => {
    ...
    const loadData = useCallback(async () => {
        const res = await GETNETWORK(buildUrl("sfa", "type=visit&limit=100"), true);
        logScreenApi("AsmVisitsScreen", "sfa", res, buildUrl("sfa", "type=visit&limit=100"));
        if (isApiSuccess(res)) {
            setRows(
                extractApiList(res).map((row, i) => ({
                    id: String(row.id ?? i),
                    party: row.party_name || row.outlet_name || row.customer_name || "—",
                    purpose: row.purpose || row.visit_type || row.activity_type || "Field visit",
                    date: row.visit_date || row.activity_date || row.created_at?.slice?.(0, 10) || "—",
                    status: row.status || "—",
                    route: row.route?.name || row.route_name || row.beat_name || "—",
                }))
            );
        }
        setRefreshing(false);
    ...
    return (
        <View style={styles.root}>
            <FinanceHeader title="Visits" profileInitial="A" onProfilePress={navigation.openDrawer} />
            <SafeAreaView style={styles.safeArea} edges={[]}>
                <FlatList
                    ...
                    ListHeaderComponent={
                        <View>
                            <Text style={styles.pageTitle}>Field Visits</Text>
                            <TextInput
                                style={styles.search}
...
export default AsmVisitsScreen;
