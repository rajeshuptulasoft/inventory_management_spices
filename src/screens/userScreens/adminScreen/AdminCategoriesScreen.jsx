...
const CategoryCard = ({ item, onEdit, onDelete }) => (
    <View style={styles.categoryCard}>
        ...
        <TouchableOpacity style={styles.deleteLink} activeOpacity={0.85} onPress={() => onDelete(item)}>
...
const AdminCategoriesScreen = () => {
    ...
    const fetchCategories = useCallback(async (isRefresh = false) => {
        ...
        try {
            const res = await GETNETWORK(buildUrl("master/categories"), true);
            logScreenApi("AdminCategoriesScreen", "master/categories", res, buildUrl("master/categories"));
            if (!isApiSuccess(res)) {
    ...
    const handleSaveCategory = async (form) => {
        ...
        let res;
        if (form.id) {
            res = await PUTNETWORK(buildUrl(`master/categories/${form.id}`), payload, true);
            logScreenApi(
                "AdminCategoriesScreen",
                "master/categories/update",
                res,
                buildUrl(`master/categories/${form.id}`)
            );
        } else {
            res = await POSTNETWORK(buildUrl("master/categories"), payload, true);
            logScreenApi("AdminCategoriesScreen", "master/categories/create", res, buildUrl("master/categories"));
        }
        if (!isApiSuccess(res)) {
    ...
    const handleDelete = (item) => {
        Alert.alert("Delete Category", `Delete "${item.name}"?`, [
            { text: "Cancel", style: "cancel" },
            {
                ...
                style: "destructive",
                onPress: async () => {
                    const res = await DELETENETWORK(buildUrl(`master/categories/${item.id}`), true);
                    logScreenApi(
                        "AdminCategoriesScreen",
                        "master/categories/delete",
                        res,
                        buildUrl(`master/categories/${item.id}`)
                    );
                    if (!isApiSuccess(res)) {
    ...
    const listHeader = (
        <View>
            <View style={styles.pageHeader}>
                <Text style={styles.pageTitle}>Master Categories</Text>
                <Text style={styles.pageSubtitle}>Organize products by category for sales and inventory</Text>
            </View>
            <View style={styles.summaryCard}>
                ...
                <Text style={styles.summaryFooter}>
                    {loading ? "→ Loading..." : loadError ? `→ ${loadError}` : "→ From master data"}
                </Text>
            </View>
    ...
    return (
        <View style={styles.root}>
            <FinanceHeader title="Categories" profileInitial="A" onProfilePress={navigation.openDrawer} />
...
const styles = StyleSheet.create({
    ...
    summaryLabel: { fontFamily: FIRASANSSEMIBOLD, fontSize: 10, color: TEXT_MUTED, letterSpacing: 0.3 },
    summaryIconWrap: {
        width: 32,
        height: 32,
        borderRadius: 8,
    ...
    categoryCardRow: { flexDirection: "row", gap: 12, alignItems: "flex-start" },
    codeCol: { width: "32%" },
    categoryCol: { flex: 1 },
    fieldLabel: {
    ...
    categoryName: { fontFamily: FIRASANSSEMIBOLD, fontSize: 15, color: TEXT_DARK, lineHeight: 20 },
    deleteLink: { alignSelf: "flex-end", marginTop: 10 },
    deleteLinkText: { fontFamily: FIRASANSSEMIBOLD, fontSize: 12, color: RED },
