...
const RecordCollectionModal = ({ visible, editingItem, onClose, onSave }) => {
    ...
    useEffect(() => {
        if (visible) {
            setReceiptNo(editingItem?.receiptId ?? "");
            setPartyId(editingItem?.partyId ?? "0");
            setAmount(editingItem?.rawAmount ?? "0");
            setMode(editingItem?.mode ?? "Cash");
            setReference(editingItem?.reference ?? "");
            setDate(editingItem?.date ?? "");
            setInvoiceId(editingItem?.invoiceId ?? "0");
            setStatus(editingItem?.status ?? "Received");
        }
    }, [visible, editingItem]);
    ...
    const handleSave = () => {
        if (!partyId.trim() || !amount.trim()) {
            Alert.alert("Required Fields", "Please enter Party ID and Amount.");
            return;
        }
        onSave({
            id: editingItem?.id,
                    ...
                    <View style={styles.modalCard}>
                        <View style={styles.modalHeader}>
                            <Text style={styles.modalTitle}>
                                {editingItem ? "Edit Collection" : "Record Collection"}
                            </Text>
                        ...
                        <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
                            ...
                            <FormField label="MODE">
                                <TouchableOpacity style={styles.pickerField} onPress={cycleMode} activeOpacity={0.85}>
            ...
            >
                <View
                    style={[
                        ...
                        item.status === "Received" && styles.statusDotReceived,
                        item.status === "Cleared" && styles.statusDotCleared,
                    ]}
                />
                <Text
                    style={[
                        ...
                        item.status === "Cleared" && styles.statusTextCleared,
                    ]}
                >
                    {item.status}
                </Text>
...
const SharedCollectionScreen = () => {
    ...
    const loadData = useCallback(async () => {
        try {
            const [collectionsRes, distributorsRes] = await Promise.all([
            ...
            const distRows = isApiSuccess(distributorsRes) ? extractApiList(distributorsRes) : [];
            setCollections(
                extractApiList(collectionsRes).map((row) => mapCollectionRow(row, distRows))
            );
        } finally {
            setRefreshing(false);
        }
    ...
    const handleSaveCollection = async (form) => {
        const payload = {
            distributor_id: Number(form.partyId),
            amount: Number(form.amount),
            payment_mode: String(form.mode || "cash").toLowerCase(),
    ...
    const listHeader = (
        <View>
            ...
            <View style={styles.pageHeader}>
                ...
                <Text style={styles.pageSubtitle}>Due, overdue & advance payment tracking</Text>
            </View>
        ...
        <View style={styles.root}>
            ...
            <SafeAreaView style={styles.safeArea} edges={[]}>
                <FlatList
                    ...
                    renderItem={({ item }) => (
                        <CollectionCard item={item} onEdit={openEditModal} onDelete={handleDelete} />
                    )}
...
const styles = StyleSheet.create({
    ...
    receiptId: { fontFamily: FIRASANSSEMIBOLD, fontSize: 13, color: PRIMARY_BLUE },
    amountValue: { fontFamily: UBUNTUBOLD, fontSize: 18, color: GREEN },
    statusBadge: {
        flexDirection: "row",
        alignItems: "center",
    ...
    statusCleared: { backgroundColor: "#ECFDF5" },
    statusDot: { width: 6, height: 6, borderRadius: 3, backgroundColor: GREEN },
    statusDotPending: { backgroundColor: AMBER },
    statusDotReceived: { backgroundColor: PRIMARY_BLUE },
    statusDotCleared: { backgroundColor: GREEN },
    statusText: { fontFamily: FIRASANSSEMIBOLD, fontSize: 11, color: GREEN },
    statusTextPending: { color: AMBER },
    statusTextReceived: { color: PRIMARY_BLUE },
    statusTextCleared: { color: GREEN },
    modeRow: { marginBottom: 14 },
    modeValue: { fontFamily: FIRASANSSEMIBOLD, fontSize: 14, color: TEXT_DARK },
