...
const AddDealerModal = ({ visible, editingItem, onClose, onSave }) => {
...
const DealerCard = ({ item, onStockLedger, onEdit, onDelete }) => {
    const isActive = item.status === "Active";
    const location = item.location ?? [item.city, item.state].filter(Boolean).join(", ");

    return (
        <View style={styles.dealerCard}>
            <View style={styles.dealerCardTop}>
...
const SharedDealerScreen = () => {
    ...
    const loadData = useCallback(async () => {
        try {
            ...
            setDealers(extractApiList(res).map((row) => mapFmcgPartnerRow(row, "DLR")));
        } finally {
            setRefreshing(false);
        }
    }, []);
    ...
    const handleSaveDealer = async (form) => {
        const payload = {
            name: form.name,
    ...
    const handleDelete = (item) => {
        Alert.alert("Delete Dealer", `Delete ${item.name} (${item.code})?`, [
        ...
        <View>
            ...
            <View style={styles.pageHeader}>
                <View style={styles.titleBlock}>
                    <Text style={styles.pageTitle}>Dealer Management</Text>
                    <Text style={styles.pageSubtitle}>
                        Dealer network, outstanding & stock ledger
                    </Text>
                </View>
            </View>
            ...
            <View style={styles.listSection}>
                <Text style={styles.sectionTitle}>Dealer Management</Text>
...
export default SharedDealerScreen;
