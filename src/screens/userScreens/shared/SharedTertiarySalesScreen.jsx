...
const TYPE_OPTIONS = ["Primary", "Secondary", "Tertiary"];
...
const SALES_TYPE = "tertiary";

const mapOrderToSalesUi = (row) => {
    const base = mapSalesOrderRow(row);
    const rawType = row.order_type || row.type || row.sales_type || "tertiary";
    return {
        id: base.id,
...
const NewOrderModal = ({ visible, editingItem, onClose, onSave }) => {
    ...
    useEffect(() => {
        if (visible) {
            ...
            setType(editingItem?.type ?? "Tertiary");
...
const SharedTertiarySalesScreen = () => {
    ...
    const loadData = useCallback(async () => {
        ...
        try {
            ...
            logScreenApi("SharedTertiarySalesScreen", "fmcg/sales-orders", res, buildUrl("fmcg/sales-orders", "limit=100"));
            if (!isApiSuccess(res)) {
    ...
    const listHeader = (
        <View>
            ...
            <View style={styles.pageHeader}>
                <Text style={styles.pageTitle}>Tertiary Sales</Text>
                <Text style={styles.pageSubtitle}>Retailer → Consumer demand tracking</Text>
            </View>
            ...
            <View style={styles.listSection}>
                <Text style={styles.sectionTitle}>Tertiary Sales</Text>
                    ...
                    ListEmptyComponent={
                        !loading ? (
                            <View style={styles.emptyWrap}>
                                <Text style={styles.emptyText}>No tertiary sales records found</Text>
                            </View>
                        ) : null
                    }
                />
...
export default SharedTertiarySalesScreen;
