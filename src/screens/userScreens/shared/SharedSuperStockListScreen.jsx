...
const AddSuperStockistModal = ({ visible, editingItem, onClose, onSave }) => {
                    ...
                    <View style={styles.modalCard}>
                        <View style={styles.modalHeader}>
                            <Text style={styles.modalTitle}>
                                {editingItem ? "Edit Super" : "Add Super"}
                            </Text>
                            <TouchableOpacity
                                onPress={handleClose}
...
const SuperStockistCard = ({ item, onStockLedger, onEdit, onDelete }) => {
    const isActive = item.status === "Active";
    const location = item.location ?? [item.city, item.state].filter(Boolean).join(", ");

    return (
        <View style={styles.stockistCard}>
            <View style={styles.stockistCardTop}>
            ...
            <View style={styles.stockistMetaRow}>
...
const SharedSuperStockListScreen = () => {
    ...
    const loadData = useCallback(async () => {
        try {
            ...
            logScreenApi("SharedSuperStockListScreen", "fmcg/wholesalers", res, buildUrl("fmcg/wholesalers", "limit=200"));
            if (!isApiSuccess(res)) {
                Alert.alert("Error", getApiMessage(res, "Failed to load super stockists"));
                return;
            }
            setStockists(extractApiList(res).map((row) => mapFmcgPartnerRow(row, "WS")));
        } finally {
            setRefreshing(false);
        }
    }, []);
    ...
    const handleSaveStockist = async (form) => {
        const payload = {
            name: form.name,
        ...
        if (form.id) {
            ...
            logScreenApi("SharedSuperStockListScreen", "fmcg/wholesalers/update", res, buildUrl(`fmcg/wholesalers/${form.id}`));
        } else {
            res = await POSTNETWORK(buildUrl("fmcg/wholesalers"), payload, true);
            logScreenApi("SharedSuperStockListScreen", "fmcg/wholesalers/create", res, buildUrl("fmcg/wholesalers"));
        }
        if (!isApiSuccess(res)) {
    ...
    const handleDelete = (item) => {
        Alert.alert("Delete Super Stockist", `Delete ${item.name} (${item.code})?`, [
            { text: "Cancel", style: "cancel" },
            {
                ...
                onPress: async () => {
                    ...
                    logScreenApi("SharedSuperStockListScreen", "fmcg/wholesalers/${item.id}", res, buildUrl(`fmcg/wholesalers/${item.id}`));
                    if (!isApiSuccess(res)) {
        ...
        <View>
            ...
            <View style={styles.pageHeader}>
                <View style={styles.titleBlock}>
                    <Text style={styles.pageTitle}>Super Stockist Management</Text>
                    <Text style={styles.pageSubtitle}>
                        Regional super stockists, supply chain & stock ledger
                    </Text>
                </View>
            </View>
            ...
            <View style={styles.summaryCard}>
                ...
                <Text style={styles.summaryValue}>{stockists.length}</Text>
                <Text style={styles.summaryFooter}>→ From database</Text>
            </View>

            <View style={styles.listSection}>
                <Text style={styles.sectionTitle}>Super Stockist Management</Text>
    ...
    return (
        <View style={styles.root}>
            ...
            <SafeAreaView style={styles.safeArea} edges={[]}>
                ...
                <AddSuperStockistModal
                    visible={showAddModal}
                    editingItem={editingItem}
                    onClose={closeModal}
                    onSave={handleSaveStockist}
                />
            </SafeAreaView>
        </View>
    );
...
export default SharedSuperStockListScreen;
