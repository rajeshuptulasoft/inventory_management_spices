...
const mapRawMaterialRow = (row) => ({
    ...
    rawPrice: String(row.cost_per_unit ?? 0),
...
const AddMaterialModal = ({ visible, editingItem, onClose, onSave }) => {
    ...
    useEffect(() => {
        if (visible) {
            ...
            setName(editingItem?.material ?? "");
            setReorderLevel(editingItem?.reorderLevel ?? "0");
            setStandardPrice(editingItem?.rawPrice ?? "0");
...
const RawMaterialCard = ({ item, onEdit, onDelete }) => (
    <View style={styles.materialCard}>
        <View style={styles.materialGrid}>
            ...
            <View style={styles.gridColMaterial}>
        ...
        <View style={styles.materialGrid}>
            <View style={styles.gridColPrice}>
...
const ProductionRawMaterialScreen = () => {
    ...
    const fetchMaterials = useCallback(async (isRefresh = false) => {
        ...
        try {
            ...
            logScreenApi("ProductionRawMaterialScreen", "raw-materials", res, buildUrl("raw-materials"));
            if (!isApiSuccess(res)) {
    ...
    const openEditModal = (item) => {
        setEditingItem({
            ...item,
            rawPrice: item.rawPrice ?? item.price.replace("₹", ""),
        });
        setShowModal(true);
    };
    ...
    const handleSaveMaterial = async (form) => {
        if (form.id) {
            Alert.alert("Edit Material", "Raw material updates are managed from the web admin.");
            return;
        }
...
export default ProductionRawMaterialScreen;
